...
- Strict, procedural, unimaginative — tightens security by enforcing rules to the letter (more patrols, stricter ID checks, tighter logs) rather than through cleverness.
- Doesn't recognise political subtext — won't notice if something is "off" unless it violates a written procedure.
- Genuinely loyal to the DOL's stated mission (order, light, prosperity) without cynicism — believes the propaganda.
- Makes Kaelen's Valerius disguise *more* dangerous to maintain, not less: a political operator might be talked around, but a procedural one will catch a disguise out via paperwork, schedules, or protocol Valerius would have known cold (passwords, sign-in logs, chain-of-command quirks).
